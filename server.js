	...
	for (var i = 0; i < data.length; i++)
	{
        if(data[i].id == _appid ||(_appid == 'all' && images.indexOf(data[i].attributes.image_name.replace(/:[^ ]+/,''))>-1) )
        {
            ...
            for (var j = 0; j < jn.attributes.port_mappings.length; j++)
            {
                ...
                {
                    var ret_json = {"appid":data[i].id,"server":ip,"server_port":service_port,"password":ss_password,"method":ss_method,"base64appid":base64_encode(data[i].id),"base64pass":base64_encode(ss_password),"base64group":base64_encode("arukas")};
